...
<form action="inserta.php" method="POST" enctype="multipart/form-data">
				...
				<td><input  name="name" type="text" class="form-control" placeholder="Nombre">
				...
				<?php
  				...
  				$today = date("Y-m-d");
				?>
				<td><input  name="role" type="text" class="form-control" value="<?php echo $today; ?>" placeholder="Fecha"></td>
...
<option hidden value="PROGRAMA">→→→PROGRAMA←←←</option>
	...
	<?php
include('class.php');
	$res2 =  mysql_query("SELECT * FROM hero ORDER BY name ASC");
...
$sql = "SELECT * FROM hero ORDER BY name ASC";

$res =  mysql_query($sql);

$sqli = "SELECT * FROM hero";
